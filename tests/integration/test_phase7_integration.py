...
class TestPhase7Integration:
    ...
    async def test_complete_pipeline_commercial_query(self, retrieval_controller):
        ...
        assert (
            "running_shoes" in response.extracted_categories
            or "marathon_gear" in response.extracted_categories
        )
        ...
        for i in range(len(response.campaigns) - 1):
            assert (
                response.campaigns[i].relevance_score >= response.campaigns[i + 1].relevance_score
            )
    ...
    async def test_complete_pipeline_blocked_query(self, retrieval_controller):
        ...
        request = RetrievalRequest(query="I want to commit suicide", context=None)
    ...
    async def test_complete_pipeline_informational_query(self, retrieval_controller):
        ...
        request = RetrievalRequest(query="What is the history of marathon running?", context=None)
    ...
    async def test_pipeline_without_context(self, retrieval_controller):
        ...
        request = RetrievalRequest(query="laptop for programming", context=None)
    ...
    async def test_pipeline_latency_target(self, retrieval_controller):
        ...
        request = RetrievalRequest(
            ...
            context=UserContext(
                age=30, gender="male", location="CA", interests=["fitness", "running"]
            ),
        )
    ...
    async def test_short_circuit_latency(self, retrieval_controller):
        ...
        request = RetrievalRequest(query="suicide help", context=None)
    ...
    async def test_concurrent_requests(self, retrieval_controller):
        ...
        requests = [RetrievalRequest(query=q, context=None) for q in queries]
    ...
    async def test_minimal_query(self, retrieval_controller):
        ...
        request = RetrievalRequest(query="a", context=None)  # Minimal valid query
    ...
    async def test_long_query(self, retrieval_controller):
        ...
        request = RetrievalRequest(
            query="running shoes for marathon training " * 10, context=None  # ~370 chars
        )
    ...
    async def test_special_characters_query(self, retrieval_controller):
        ...
        request = RetrievalRequest(query="running shoes!!! @#$% best??? 2024", context=None)
    ...
    async def test_campaign_relevance_scores(self, retrieval_controller):
        ...
        request = RetrievalRequest(query="running shoes", context=None)
        ...
        if len(response.campaigns) > 0:
            ...
            for i in range(len(response.campaigns) - 1):
                assert (
                    response.campaigns[i].relevance_score
                    >= response.campaigns[i + 1].relevance_score
                )

    @pytest.mark.asyncio
    async def test_campaign_fields_populated(self, retrieval_controller):
        """Test that campaign fields are properly populated."""
        request = RetrievalRequest(query="laptop computer", context=None)
    ...
    async def test_max_campaigns_returned(self, retrieval_controller):
        ...
        request = RetrievalRequest(
            query="product", context=None  # Generic query to get many results
        )
