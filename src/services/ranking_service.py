...
class RankingService:
    ...
    async def rank(
        self, campaigns: list[dict], query: str, categories: list[str], context: dict | None = None
    ) -> list[dict]:
        """
    ...
    def _apply_category_boosts(
        self, campaign: dict, categories: list[str], base_score: float
    ) -> float:
        """
        Apply category matching boosts.
    ...
    def _apply_context_boosts(self, campaign: dict, context: dict, base_score: float) -> float:
        """
        Apply context-based targeting boosts.
