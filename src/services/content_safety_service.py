...
class ContentSafetyService:
    ...
    def validate_query(self, query: str) -> tuple[bool, SafetyViolation | None, str | None]:
        """
        ...
        if self._contains_illegal_items(query):
            return (
                False,
                SafetyViolation.ILLEGAL_ITEMS,
                "Query contains references to illegal items",
            )
        ...
        if self._contains_security_threats(query):
            return (
                False,
                SafetyViolation.SECURITY_THREAT,
                "Query contains potential security threats",
            )
    ...
    def _compile_illegal_patterns(self) -> list[re.Pattern]:
        ...
        patterns = [
            ...
            re.compile(
                r"\b(buy|purchase|need|want|sell)\s+(illegal\s+)?(gun|guns|firearm|firearms|weapon|weapons)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(illegal|unregistered)\s+(gun|guns|firearm|firearms|weapon|weapons)\b",
                re.IGNORECASE,
            ),
            # Explosives - with intent context
            re.compile(
                r"\b(buy|make|build|create)\s+(explosive|explosives|bomb|bombs)\b", re.IGNORECASE
            ),
            re.compile(r"\b(pipe\s+bomb|car\s+bomb|suicide\s+bomb)\b", re.IGNORECASE),
            # Illegal drugs - with clear intent
            re.compile(
                r"\b(buy|purchase|sell|deal)\s+(cocaine|heroin|methamphetamine|meth|crack|drugs)\b",
                re.IGNORECASE,
            ),
            re.compile(r"\b(illegal\s+(drug|drugs|substance|substances))\b", re.IGNORECASE),
            re.compile(
                r"\b(where\s+to\s+buy|how\s+to\s+get)\s+(cocaine|heroin|meth|drugs)\b",
                re.IGNORECASE,
            ),
        ]
        return patterns
    ...
    def _compile_malicious_url_patterns(self) -> list[re.Pattern]:
        ...
        patterns = [
            ...
            re.compile(
                r"https?://[^\s]*?(malware|virus|phishing|steal|hack|crack|keylog)", re.IGNORECASE
            ),
