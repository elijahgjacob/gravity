...
class EligibilityService:
    ...
    async def score(self, query: str, context: dict | None = None) -> tuple[float, str | None]:
        """
    ...
    def _compile_sensitive_patterns(self) -> list[re.Pattern]:
        ...
        patterns = [
            ...
            re.compile(
                r"\b(fired|laid off|layoff|terminated|let go|lost (my|our) job)\b", re.IGNORECASE
            ),
            re.compile(r"\b(can\'?t find (a |)job|struggling to find work)\b", re.IGNORECASE),
            # Financial distress - debt and bankruptcy
            re.compile(
                r"\b(bankruptcy|bankrupt|filing for bankruptcy|going bankrupt)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(debt collector|collection agency|repossession|foreclosure)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(can\'?t pay|behind on payments|overdue payments|defaulting on)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(financial crisis|financial trouble|money problems|broke)\b", re.IGNORECASE
            ),
            ...
            re.compile(
                r"\b(lost (my|our|a) (mom|dad|mother|father|parent|son|daughter|child|baby|husband|wife|spouse|partner|friend|pet))\b",
                re.IGNORECASE,
            ),
            re.compile(r"\b(widow|widower|orphan)\b", re.IGNORECASE),
            # Mental health crises - depression and anxiety
            re.compile(
                r"\b(depressed|depression|severely depressed|clinically depressed)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(anxious|anxiety|severe anxiety|panic attack|panic disorder)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(mental breakdown|nervous breakdown|mental health crisis)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(suicidal thoughts|suicidal ideation|contemplating suicide)\b", re.IGNORECASE
            ),
            # Mental health crises - distress signals
            re.compile(
                r"\b(can\'?t cope|can\'?t handle|overwhelmed|breaking down)\b", re.IGNORECASE
            ),
            re.compile(r"\b(feeling hopeless|feel hopeless|losing hope|no hope)\b", re.IGNORECASE),
            re.compile(
                r"\b(want to die|wish (i|I) was dead|don\'?t want to live)\b", re.IGNORECASE
            ),
            ...
            re.compile(
                r"\b(choking|can\'?t breathe|difficulty breathing|respiratory distress)\b",
                re.IGNORECASE,
            ),
            ...
            re.compile(
                r"\b(need|require|seeking|looking for)\s+((urgent|severe|serious|critical|immediate|emergency)\s+)?(medical|health|doctor|hospital|physician)\s+(help|care|assistance|attention|treatment)\b",
                re.IGNORECASE,
            ),
            re.compile(r"\b(medical|health)\s+(emergency|crisis|urgency)\b", re.IGNORECASE),
            re.compile(
                r"\b(i\'?m\s+)?(sick|ill|injured|hurt|in pain)\s+(and\s+)?(need|require|want)\s+(help|doctor|hospital|treatment|care)\b",
                re.IGNORECASE,
            ),
            # Domestic violence and abuse
            re.compile(
                r"\b(domestic violence|domestic abuse|being abused|abusive relationship)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(physical abuse|emotional abuse|verbal abuse|sexual abuse)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(restraining order|protection order|safe house|women\'?s shelter)\b",
                re.IGNORECASE,
            ),
            # Addiction and substance abuse (seeking help)
            re.compile(
                r"\b(addiction help|rehab|rehabilitation|detox|withdrawal)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(alcoholic|alcohol abuse|drinking problem|substance abuse)\b", re.IGNORECASE
            ),
            re.compile(r"\b(overdosed|overdosing|drug problem)\b", re.IGNORECASE),
            # Legal troubles
            re.compile(
                r"\b(arrested|going to jail|facing charges|criminal charges)\b", re.IGNORECASE
            ),
            re.compile(r"\b(court case|lawsuit|being sued|legal trouble)\b", re.IGNORECASE),
            # Pregnancy complications and loss
            re.compile(
                r"\b(miscarriage|stillbirth|pregnancy loss|lost (the |my )baby)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(pregnancy complications|high-risk pregnancy|ectopic pregnancy)\b",
                re.IGNORECASE,
            ),
        ]
        return patterns
    ...
    def _compile_commercial_patterns(self) -> list[re.Pattern]:
        ...
        patterns = [
            ...
            re.compile(
                r"\b(looking for)\s+(a |an |new |the |some |)?(laptop|phone|computer|car|shoes|headphones|watch|camera|tv|tablet|product|item)\b",
                re.IGNORECASE,
            ),
            ...
            re.compile(
                r"\b(deal|deals|discount|discounts|sale|sales|coupon|promo)\b", re.IGNORECASE
            ),
