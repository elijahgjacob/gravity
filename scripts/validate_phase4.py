...
class Phase4Validator:
        ...
        try:
            ...
            for cat_name, cat_data in taxonomy.items():
                if not isinstance(cat_data, dict):
                    self.print_test(
                        f"Category '{cat_name}' has valid structure", False, "Not a dictionary"
                    )
                ...
                if "keywords" not in cat_data:
                    self.print_test(
                        f"Category '{cat_name}' has keywords", False, "Missing 'keywords' field"
                    )
                    valid_structure = False
                    continue
                ...
                if "related" not in cat_data:
                    self.print_test(
                        f"Category '{cat_name}' has related terms", False, "Missing 'related' field"
                    )
                    valid_structure = False
                    continue
            ...
            self.print_test(
                f"All {categories_checked} categories have valid structure", valid_structure
            )
    ...
    def validate_taxonomy_repository(self) -> bool:
        ...
        try:
            ...
            self.print_test(
                "get_category_count() returns valid count", count > 0, f"Count: {count}"
            )
            ...
            self.print_test(
                "get_category() returns empty dict for non-existent category", invalid == {}
            )
    ...
    async def validate_category_service(self) -> bool:
        ...
        try:
            ...
            categories_with_context = await service.extract("best fitness tracker", context)
            self.print_test(
            ...
            limited = await service.extract(
                "fitness health wellness sports running yoga", max_categories=3
            )
            self.print_test(
    ...
    async def validate_integration_scenarios(self) -> bool:
        ...
        try:
            ...
            for scenario in test_scenarios:
                categories = await service.extract(scenario["query"], scenario["context"])

                has_match = any(exp in categories for exp in scenario["expected_contains"])
    ...
    async def validate_performance(self) -> bool:
        ...
        try:
            ...
            if not meets_target:
                self.print_warning(f"Latency {latency_ms:.2f}ms exceeds target {target_latency}ms")
            ...
            self.print_test(
                "Concurrent extractions complete",
