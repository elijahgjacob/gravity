...
def benchmark_latency(
    ...
    """
    print(f"\n{'='*60}")
    print(f"  Ad Retrieval API Benchmark")
    print(f"{'='*60}\n")
...
def print_results(stats: Dict):
    ...
    print(f"P95:                 {stats['p95_ms']:>10.2f}ms  {'✅ Target: <100ms' if stats['p95_ms'] < 100 else '❌ Target: <100ms'}")
    ...
    if 'short_circuit_mean_ms' in stats:
        ...
        print(f"{'-'*60}")
        print(f"Short-Circuit Avg:   {stats['short_circuit_mean_ms']:>10.2f}ms  ({stats['short_circuit_count']} requests)")
        print(f"Full Pipeline Avg:   {stats['full_pipeline_mean_ms']:>10.2f}ms  ({stats['full_pipeline_count']} requests)")
