...
def check_pydantic_models():
    ...
    try:
        ...
        from src.api.models.responses import RetrievalResponse, Campaign, HealthResponse
        
        # Test UserContext
        context = UserContext(age=25, gender="male")
        print(f"   ✅ UserContext model works")
        
        # Test RetrievalRequest
        request = RetrievalRequest(query="test query", context=context)
        print(f"   ✅ RetrievalRequest model works")
        ...
        print(f"   ✅ RetrievalResponse model works")
        
        # Test HealthResponse
        health = HealthResponse(status="healthy", version="1.0.0")
        print(f"   ✅ HealthResponse model works")
...
def check_timing_utilities():
    ...
    try:
        from src.utils.timing import timer, LatencyTracker
        import time
        
        # Test timer context manager
        with timer("test operation"):
            time.sleep(0.01)
        print(f"   ✅ Timer context manager works")
